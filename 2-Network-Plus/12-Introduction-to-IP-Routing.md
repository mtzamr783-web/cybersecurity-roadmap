...
! على R3: للوصول إلى شبكة 172.16.0.0 (البوابة التالية = منفذ R2 المتصل بـR3 مباشرة، وليس R1 لأن R3 غير متصل به مباشرة)
...
هذان مصطلحان **متشابهان جدًا في الاسم** (كلاهما يحتوي على كلمة "Routing" أو مشتقاتها)، وهذا بالضبط سبب اللبس الشائع بينهما رغم أن وظيفتيهما مختلفتان تمامًا. أي متدرب في مجال الشبكات يمر بمرحلة الخلط بينهما في البداية، لذلك من المهم توضيح الفرق بدقة:

| المقارنة | Routing Protocol (بروتوكول التوجيه) | Routed Protocol (بروتوكول موجَّه) |
|:---:|:---:|:---:|
...
| ما الذي يتم نقله؟ | معلومات **عن** الشبكات (إعلانات مسارات، Metric، Hop Count...) | البيانات **الفعلية** الخاصة بالمستخدم (صفحة ويب، ملف، رسالة بريد...) |
| هل له عنونة خاصة به؟ | لا؛ يعمل فوق البروتوكول الموجَّه ليستخدم نفس آلية العنونة (IP) في نقل رسائله | نعم؛ هو نفسه من يحدد نظام العنونة (IP Addressing) الذي يعتمد عليه التوجيه بالكامل |
| مثال | RIP, EIGRP, OSPF, BGP | IP (IPv4 / IPv6) — وتاريخيًا: IPX/SPX, AppleTalk |
| العلاقة بينهما | يعمل "خلف الكواليس" لتجهيز المسار قبل وصول أي بيانات فعلية | يستخدم المسار الجاهز الذي بناه بروتوكول التوجيه ليوصّل بياناته فعليًا |

### مثال يوضح الفرق عمليًا

تخيل راوترين R1 و R2 متصلين ببعض، وعليهما بروتوكول **OSPF** يعمل:

1. **أولًا (Routing Protocol):** يتبادل R1 وR2 رسائل OSPF فيما بينهما (Hello Messages, LSAs) لبناء جدول التوجيه لدى كل منهما، ومعرفة أفضل مسار لكل شبكة. هذه الرسائل **لا تحمل أي بيانات مستخدم**، هي فقط "معلومات عن الشبكة نفسها".
2. **بعد ذلك (Routed Protocol):** عندما يرسل مستخدم فعلي طلب تصفح لموقع ويب، تُغلَّف هذه البيانات داخل حزمة **IP** (وهو البروتوكول الموجَّه)، ويستخدم الراوتر جدول التوجيه الذي بناه OSPF مسبقًا لتحديد إلى أين يوجّه حزمة الـIP هذه.

بمعنى آخر: **OSPF لا ينقل بيانات المستخدم إطلاقًا**، ودوره ينتهي بمجرد تجهيز جدول التوجيه؛ أما **IP** فهو من يحمل كل بيانات المستخدم الفعلية معتمدًا على المسار الذي جهّزه OSPF.

### سبب اللبس الشائع ونقطة الحسم

اللبس الأكبر يحدث لأن كلمة "Protocol" مشتركة بين الاثنين، وكلاهما مرتبط بمفهوم "التوجيه"، فيسهل الظن بأنهما نفس الشيء أو مترادفان. **نقطة الحسم البسيطة لإزالة اللبس نهائيًا:**

> إذا كان البروتوكول **يوجّه** بيانات غيره ولا ينقل بيانات مستخدم بنفسه ← فهو **Routing Protocol**.
> إذا كان البروتوكول هو نفسه **البيانات التي يتم توجيهها** ← فهو **Routed Protocol**.

بصيغة مبسطة أخيرة: **بروتوكول التوجيه (Routing Protocol) يبني الخريطة (جدول التوجيه)، بينما البروتوكول الموجَّه (Routed Protocol) هو البيانات التي تسافر فعليًا على هذه الخريطة.**
...
11. راوتران يستخدمان OSPF ولديهما مساران مختلفان لنفس الشبكة الوجهة. على أي أساس سيختار الراوتر أفضل مسار بينهما؟ وما الفرق بين هذا الأساس ومقارنة AD؟
12. اشرح كيف تحدث حلقة توجيه (Routing Loop) في بروتوكول RIP بعد انقطاع رابط، ولماذا تتوقف تلقائيًا عند 16 قفزة؟
13. ما الفرق بين Split Horizon و Poison Reverse؟
14. لديك Static Route وEIGRP لنفس الشبكة، وتريد أن يبقى EIGRP هو المسار الأساسي مع الاحتفاظ بالمسار الساكن كاحتياطي فقط. ما القيمة التي يجب ضبطها على المسار الساكن، ولماذا؟
15. لماذا تُعتبر بروتوكولات OSPF وEIGRP أسرع تقاربًا من RIP بشكل عام؟
16. أربع شبكات فرعية متتالية بقناع /24 تبدأ من 10.10.0.0. اقترح مسارًا واحدًا ملخَّصًا (Summarized Route) يغطيها جميعًا.
17. لماذا يحتاج الراوتر الذي يقوم بـ NAT لمسار افتراضي (Default Route) بشكل شبه دائم؟
